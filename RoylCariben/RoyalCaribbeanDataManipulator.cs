...
namespace RoyalCaribbeanPlugin
{
    public class RoyalCaribbeanDataManipulator : DataManipulator
    {
     ...
     VALUES
           ...
           ,@sequence_nuber
           ,@non_refundable)";
        const string insPriceEu = @"INSERT INTO [dbo].[Rccl_price_eu]
           ([Package]
           ,[Sail_date]
           ,[fare_code]
           ,[roomCategory]
     ...
     VALUES
                ...
                using (StreamReader sr = new StreamReader(priceRCCL))
                {
                    ...
                    for (int i = 1; i < items.Length - 1; i++)
                    {
                        ...
                        if (!string.IsNullOrEmpty(item))
                            if (item.Substring(item.Length - 1) == "\r") 
                        ...
                        using (SqlCommand com = new SqlCommand(insPrice, _connection))
                        {
                            ...
                            string ss = "";
                            if (s.Length >= 65)
                                if (!string.IsNullOrEmpty(s[64])) ss = s[64];
                            com.Parameters.AddWithValue("@non_refundable", ss);
                            com.ExecuteNonQuery();
        ...
        void GetPricingPullman()
        {
            ...
            try
            {
                ...
                //--------------------------------------------------------------------
                //------------------------------------
                Uri PriceUri = new Uri("ftp://" + server + ":" + port.ToString() + "/EP1_RUE_EUR/EP1_RUE_EUR_cruise_no_air_price.zip");
                FtpWebRequest PriceFtpReq = (FtpWebRequest)FtpWebRequest.Create(PriceUri);
                Stream priceftpStream = null;
                FtpWebResponse priceresponse = null;
                PriceFtpReq.Method = WebRequestMethods.Ftp.DownloadFile;
                ...
                PriceFtpReq.EnableSsl = true;
                PriceFtpReq.Credentials = new NetworkCredential(user, pass);
                _logFile.WriteLine("RC : EP1_RUE_EUR_cruise_no_air_price.zip Начало закачки");
                PriceFtpReq.Timeout = 600000;
                priceresponse = (FtpWebResponse)PriceFtpReq.GetResponse();
                priceftpStream = priceresponse.GetResponseStream();
                //Stream stream = new MemoryStream();
                ...
                Stream priceRCCL = UnpackZip(priceftpStream);
                SqlCommand delCom = new SqlCommand("Delete from Rccl_price_eu ", _connection);
                delCom.ExecuteNonQuery();
                ...
                using (StreamReader sr = new StreamReader(priceRCCL))
                {
                    ...
                    for (int i = 1; i < items.Length - 1; i++)
                    {
                        ...
                        var s = item.Split(new char[] { '|' });
                        using (SqlCommand com = new SqlCommand(insPriceEu, _connection))
                        {
